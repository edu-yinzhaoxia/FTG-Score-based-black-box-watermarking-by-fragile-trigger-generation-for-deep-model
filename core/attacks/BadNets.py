...
class PoisonedCIFAR10(CIFAR10):
    ...
    def __getitem__(self, index):
        ...
        return img, target
###########################################################################
class PoisonedTinyImageNet(MyTinyImageNet):
    def __init__(self,
                 benign_dataset,
                 y_target,
                 poisoned_rate,
                 ...
                 poisoned_transform_index,
                 poisoned_target_transform_index):
        super(PoisonedTinyImageNet, self).__init__(
            benign_dataset.root,
            benign_dataset.is_train,
            benign_dataset.transform)
        total_num = len(benign_dataset)
        poisoned_num = int(total_num * poisoned_rate)
        assert poisoned_num >= 0, 'poisoned_num should greater than or equal to zero.'
...
def CreatePoisonedDataset(benign_dataset, y_target, poisoned_rate, pattern, weight, poisoned_transform_index, poisoned_target_transform_index):
    ...
    if class_name == DatasetFolder:
        ...
        return PoisonedCIFAR10(benign_dataset, y_target, poisoned_rate, pattern, weight, poisoned_transform_index, poisoned_target_transform_index)
    elif class_name == MyTinyImageNet:
        return PoisonedTinyImageNet(benign_dataset, y_target, poisoned_rate, pattern, weight, poisoned_transform_index, poisoned_target_transform_index)
    else:
