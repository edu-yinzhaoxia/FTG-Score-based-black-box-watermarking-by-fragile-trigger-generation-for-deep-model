...
from ..Mydataloader.MyTinyImageNet import *
...
class Base(object):
    ...
    def train(self, schedule=None):
        ...
        work_dir = osp.join(self.current_schedule['save_dir'], self.current_schedule['experiment_name'] + '_' + time.strftime("%Y-%m-%d_%H_%M_%S", time.localtime()))
        ...
        for i in range(self.current_schedule['epochs']):
            ...
            for batch_id, batch in enumerate(train_loader):
                ...
                if iteration % self.current_schedule['log_iteration_interval'] == 0:
                    ...
                    msg = time.strftime("[%Y-%m-%d_%H:%M:%S] ",
                                        time.localtime()) + f"Epoch:{i + 1}/{self.current_schedule['epochs']}, iteration:{batch_id + 1}/{len(self.train_dataset) // self.current_schedule['batch_size']}, lr: {self.current_schedule['lr']}, loss: {float(loss)}, time: {time.time() - last_time}\n"
            ...
            if (i + 1) % self.current_schedule['test_epoch_interval'] == 0:
                ...
                ###今天用完恢复
                # predict_digits, labels = self._test(self.poisoned_test_dataset, device, self.current_schedule['batch_size'])
                # total_num = labels.size(0)
                # prec1, prec5 = accuracy(predict_digits, labels, topk=(1, 5))
                # top1_correct = int(round(prec1.item() / 100.0 * total_num))
                # top5_correct = int(round(prec5.item() / 100.0 * total_num))
    ...
    def _test(self, dataset, device, batch_size=16, model=None):
        if model is None:
            model = self.model
        else:
            model = model
    ...
    def test(self, schedule=None, model=None, test_dataset=None, poisoned_test_dataset=None):
        ...
        if test_dataset is not None:
            last_time = time.time()
            # test result on benign test dataset
            predict_digits, labels = self._test(test_dataset, device, self.current_schedule['batch_size'], model)
            total_num = labels.size(0)
            prec1, prec5 = accuracy(predict_digits, labels, topk=(1, 5))
            top1_correct = int(round(prec1.item() / 100.0 * total_num))
            top5_correct = int(round(prec5.item() / 100.0 * total_num))
        ...
        if poisoned_test_dataset is not None:
            last_time = time.time()
            # test result on poisoned test dataset
            predict_digits, labels = self._test(poisoned_test_dataset, device, self.current_schedule['batch_size'], model)
            total_num = labels.size(0)
            prec1, prec5 = accuracy(predict_digits, labels, topk=(1, 5))
            top1_correct = int(round(prec1.item() / 100.0 * total_num))
            top5_correct = int(round(prec5.item() / 100.0 * total_num))
